...
export const metadata = {
  title: 'Sponsor Dashboard | DAFT Arena Sponsor',
  description: 'Central hub for KPIs, active campaigns, and ROI overview.',
};

export default async function SponsorSponsorDashboardPage() {
  const data = await SponsorService.getMockData('page.tsx');
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-10 max-w-6xl">
      ...
      <WidgetContainer className="p-12 text-center flex flex-col items-center justify-center border-dashed bg-card/10">
        <div className="w-16 h-16 rounded-full bg-blue-500/10 flex items-center justify-center mb-4">
          <LayoutDashboard className="w-8 h-8 text-blue-400 opacity-80" />
        </div>
        <h3 className="text-sm font-bold text-foreground mb-1">Sponsor Dashboard Module</h3>
