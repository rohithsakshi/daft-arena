...
export const metadata = {
  title: 'Sponsor Settings | DAFT Arena Sponsor',
  description: 'Manage company profile, billing, contacts, and notifications.',
};

export default async function SponsorSponsorSettingsPage() {
  const data = await SponsorService.getMockData('settings');
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-10 max-w-6xl">
      ...
      <WidgetContainer className="p-12 text-center flex flex-col items-center justify-center border-dashed bg-card/10">
        <div className="w-16 h-16 rounded-full bg-blue-500/10 flex items-center justify-center mb-4">
          <Settings className="w-8 h-8 text-blue-400 opacity-80" />
        </div>
        <h3 className="text-sm font-bold text-foreground mb-1">Sponsor Settings Module</h3>
