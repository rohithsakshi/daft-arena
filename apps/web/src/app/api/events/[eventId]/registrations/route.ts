...
export const GET = async (req: NextRequest, { params }: { params: Promise<{ eventId: string }> }) => {
  ...
  const result = await registrationService.listRegistrations({ page, limit, eventId });
  return NextResponse.json(result, { status: 200 });
};

export const POST = withPermission('MANAGE_REGISTRATIONS', async (req: NextRequest, user: { sub: string }, { params }: { params: Promise<{ eventId: string }> }) => {
  try {
    const { eventId } = await params;
    const body = await req.json();
    const data = CreateRegistrationSchema.parse({ ...body, eventId });
    const registration = await registrationService.createRegistration(data.tournamentId, eventId, data as never, user.sub);
    ...
    if (error instanceof BusinessRuleException || error instanceof ConflictException) return NextResponse.json({ error: (error as Error).message }, { status: 400 });
