...
export function LoginForm() {
  ...
  function resolveRedirect(role: string, onboardingCompleted: boolean, isNewUser?: boolean) {
    if (!onboardingCompleted || isNewUser) return '/onboarding';
    if (callbackUrl && callbackUrl.startsWith('/')) return callbackUrl;
    const r = role?.toUpperCase();
    if (r === 'PLAYER') return '/workspace/player';
    if (r === 'SPONSOR') return '/workspace/sponsor';
    return '/workspace';
  }
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        toast.success('Welcome to DAFT Arena!');
        const redirect = resolveRedirect(data.data?.user?.systemRole, true, data.data?.isNewUser);
  ...
  const onSubmit = async (data: LoginValues) => {
    ...
    try {
      ...
      form.setError('password', { message: ' ' }); // Highlight password field
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="text-xs font-medium text-violet-400 hover:text-violet-300 hover:underline transition-colors">
              Forgot password?
            </button>
          </div>
