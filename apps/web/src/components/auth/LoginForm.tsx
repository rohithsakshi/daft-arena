    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (data.data.isNewUser) {
          router.push('/workspace/player/settings');
        } else if (callbackUrl) {
    ...
    <div className="h-full flex flex-col justify-center p-5 md:p-7 rounded-2xl border border-white/5 bg-card/60 backdrop-blur-xl shadow-2xl">
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between">
            ...
            <button type="button" className="text-xs font-medium text-violet-400 hover:text-violet-300 hover:underline transition-colors">Forgot password?</button>
          </div>
        ...
        <Button 
          type="submit" 
          className="w-full h-11 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-white rounded-lg shadow-[0_0_15px_rgba(124,58,237,0.2)] hover:shadow-[0_0_25px_rgba(124,58,237,0.4)] transition-all duration-300 border-0 mt-2" 
          disabled={isLoading}
        >
      ...
      <div className="grid grid-cols-2 gap-3">
        <Button 
          ...
          className="h-11 bg-white/5 border-white/10 hover:bg-white/10 hover:text-foreground transition-all rounded-lg gap-2 text-sm"
        >
          {isGoogleLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <svg viewBox="0 0 24 24" className="w-4 h-4"><path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" /><path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" /><path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" /><path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" /></svg>}
          Google
        </Button>
        <div className="relative group">
          <Button 
            ...
            type="button" 
            disabled
            className="w-full h-11 bg-white/5 border-white/10 opacity-60 cursor-not-allowed rounded-lg gap-2 text-sm"
          >
          ...
          <div className="absolute -top-2 -right-2 bg-primary text-[9px] font-bold px-1.5 py-0.5 rounded shadow-sm text-primary-foreground tracking-wide pointer-events-none">
            SOON
          </div>
        </div>
      </div>
